...
void main() {
   ...
   while(1){
     ...
     /*while( respons_success() != 1 );
      delay_ms(1000);

       delay_ms(5000);
       UART1_write_text_const(AT_RST);
        new_line();
        delay_ms(3000);
           UART1_write_text_const(AT_CWMODE);
        new_line();
        delay_ms(2000);

           UART1_write_text_const( AT_CWJAP);
        new_line();
        delay_ms(2000);
           UART1_write_text_const(AT_CWLAP);
        new_line();
        delay_ms(2000);
           UART1_write_text_const(AT_CWQAP);
        new_line();
        delay_ms(2000);
        UART1_write_text_const(AT_CIPMUX);
        new_line();*/
        delay_ms(2000);
...
/*Lcd_Init();
   ...
   delay_ms(1000);
   lcd_cmd(_LCD_CLEAR);*/
   trisd.B0=0;    // tihs pin is an output pin for reset wifi
...
void get_ip_mac(){
  ...
  void wifi_config (){
   ...
   lcd_out(1,2,"please wait.....");
    // reset_wifi();
     uart1_write_at("AT");
 while( respons_success() !=OK);    //READY
     delay_ms(1000);
       lcd_cmd(_LCD_CLEAR);
